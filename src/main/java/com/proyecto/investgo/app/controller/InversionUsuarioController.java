...
public class InversionUsuarioController {
	...
	public ResponseEntity<?> buscar(@PathVariable String codigo, Paginador paginador, HttpSession session) {
	...
	public ResponseEntity<?> buscarInvercionesXOportunidadUsuario(@PathVariable String codigoOpo, Paginador paginador,
		...
		Usuario usuSession = (Usuario) session.getAttribute("usuarioSession");
		Page<InversionUsuarioDTO> lista = inversionUsuarioservice
				.findByOportunidadInversionIdAndUsuarioIdDto(opoInvOp.get().getId(), usuSession.getId(), paginador);
		return ResponseEntity.ok(lista);
	}
	...
	public ResponseEntity<?> buscarInvercionesXUsuarioPagadas(
		...
		Page<InversionUsuarioDTO> lista = inversionUsuarioservice
				.findByUsuarioIdAndAuditoriaEnableDto(usuSession.getId(), valorEnable, paginador);
		return ResponseEntity.ok(lista);
	}
	...
	public ResponseEntity<?> guardar(@RequestBody @Valid InversionUsuario objOpUsu, BindingResult result,
		...
		Usuario usuSession = (Usuario) session.getAttribute("usuarioSession");
		try {
			Optional<Cartera> cartera = carteraService.findByUsuarioId(usuSession.getId());
			...
			Optional<Usuario> usuOp = usuarioService.findById(usuSession.getId());
