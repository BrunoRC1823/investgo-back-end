...
public class UsuarioController {
    ...
    public ResponseEntity<?> buscarPorCodigo(@PathVariable String codigo, HttpServletRequest request) {
        Map<String, Object> salida = new HashMap<>();
        String idU = decoderService.getIdUser(request);
        Boolean esPropietario = usuarioService.validarPropietarioUsuario(idU, codigo);
        if (!esPropietario) {
    ...
    public ResponseEntity<?> actualizarPassword(@RequestBody Map<String, Object> Orequest, BindingResult result,
                                                HttpServletRequest request) {
        Map<String, Object> salida = new HashMap<>();
        String passwordActual = (String) Orequest.get("passwordActual");
        String passwordNuevo = (String) Orequest.get("passwordNuevo");
        String idU = decoderService.getIdUser(request);
        try {
            Optional<Usuario> opUsu = usuarioService.findByCodigo(idU);
            if (opUsu.isEmpty()) {
                salida.put("mensaje", "El usuario no existe!");
                return new ResponseEntity<>(salida, HttpStatus.NOT_FOUND);
            }
            boolean passIgual = usuarioService.validarPassword(idU, passwordActual);
            if (!passIgual) {
            ...
            boolean usuarioNew = usuarioService.updateUserPassword(opUsu.get(), passwordNuevo);
            if (!usuarioNew) {
    ...
    public ResponseEntity<?> actualizarUsuario(@RequestBody @Valid Usuario usuario, BindingResult result,
        ...
        Map<String, Object> salida = new HashMap<>();
        String idU = decoderService.getIdUser(request);
        Boolean esPropietario = usuarioService.validarPropietarioUsuario(idU, usuario.getCodigo());
        if (!esPropietario) {
            salida.put("mensaje", "Ocurrio un error!");
            return new ResponseEntity<>(salida, HttpStatus.NOT_FOUND);
        }
    ...
    public ResponseEntity<?> deshabilitar(HttpServletRequest request) {
        ...
        try {
            ...
            boolean usuarioDes = usuarioService.deshabilitarUser(opUsu.get());
            if (!usuarioDes) {
    ...
    public ResponseEntity<?> buscar(HttpServletRequest request) {
        ...
        String idU = decoderService.getIdUser(request);
        Optional<CarteraDTO> cartera = carteraService.findByUsuarioIdDto(idU);
        if (cartera.isEmpty()) {
